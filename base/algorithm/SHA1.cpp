...
void CSHA1::ReportHash(char *szReport, unsigned char uReportType)
{
	...
	{
		snprintf(szTemp, sizeof(szTemp)-1, "%02X", m_digest[0]);
		strcat(szReport, szTemp);

		for(i = 1; i < 20; i++)
		{
			snprintf(szTemp, sizeof(szTemp)-1, " %02X", m_digest[i]);
			strcat(szReport, szTemp);
		}
	}
	else if(uReportType == REPORT_DIGIT)
	{
		snprintf(szTemp, sizeof(szTemp)-1, "%u", m_digest[0]);
		strcat(szReport, szTemp);

		for(i = 1; i < 20; i++)
		{
			snprintf(szTemp, sizeof(szTemp)-1, " %u", m_digest[i]);
			strcat(szReport, szTemp);
